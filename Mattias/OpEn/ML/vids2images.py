...
if not remove_only and make_pos_images:
    with open(POS_CVS_FILE, "w") as pos_annot:
        ...
        for a in range(45):
            for i, xi in enumerate(range(0, size_w_merge-w_car+1, int(size_w_merge/20))):
                for j, yi in enumerate(range(0, size_h_merge-h_car+1, int(size_h_merge/20))):
...
if not remove_only and make_theta_images:
    with open(THETA_CVS_FILE, "w") as theta_annotation:
        ...
        for j in range(1):
            for i, thi in enumerate(range(0, 360, 20)):
                for background_path in theta_back_paths:
                    ...
                    img = np.array(background_pil)
                    img = img[rand_y:int((rand_y+h_car*1.2)),
                            rand_x:int((rand_x+w_car*1.2))]
                    num = '0'*(5-len(str(im_num)))+str(im_num)
