...
# X.sort()
# Y.sort()
# image = hejpa[int(y-h_car_orig*0.2):int((y+h_car_orig*1.2)), int(x-w_car_orig*0.2):int((x+w_car_orig*1.2))]

# x, xp = inperpolate(mlb_pos.classes_, proba, X)
# y, yp = inperpolate(mlb_pos.classes_, proba, Y)
# #### THETA
# # image = hejpa[int(y-h_car*0.2):int((y+h_car*1.2+h_car*0.2)), int(x-w_car*0.2):int((x+w_car*1.2+w_car*0.2))]
