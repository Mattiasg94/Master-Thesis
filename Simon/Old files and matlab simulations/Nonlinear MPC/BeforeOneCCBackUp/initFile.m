...
xr = [Length; 0.5; -pi/4];      % Reference state
...
Q=[600 0 0;          % x state weight
    0 600 0;         % y state weight
    0 0 1];          % theta state weight
R=[1 0;              % v input weight
    0 1];            % w input weight
R_jerk = 3;          % Term that penalizes the high amounts of jerk [accelerations]
...
%% OBSTACLES
obstacles_lanes = {[2],[1]};        % Lane numbers of obstacles
