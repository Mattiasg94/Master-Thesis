...
for i = 0:N-1
    ...
    for j = 1:length(obstacles)
        ...
        vx_obs = v_obs*cos(obstacles{j}(3));
        vy_obs = v_obs*sin(obstacles{j}(3));
        ego_dist = sqrt((x-xr(1))^2+(y-xr(2))^2);
        obs_dist = sqrt((x_obs-xr(1))^2+(y_obs-xr(2))^2);    

        %% Cost lane_cost
        lane1 = (lane_border_min+(lanewidth/2));
        lane2 = (lane_border_min+(lanewidth/2))+lanewidth;
        dist_ymin = abs(lane1-radius_ego)*12;
        dist_ymax = abs(radius_ego-lane2)*12;
%         if (lane_border_max-radius_ego) < lanewidth/2
...
%         end
    ...
    if all(ismember(deactivated_cone,1)) % If both cone for both obstacles is OFF, track reference lane!
        % This needs to be very large in order to converge back to the
        % reference lane. 
        lane_keep_cost = lane_keep_cost + 10000*(radius_ego - lines_st.reference_lane)^2;
    else
        lane_keep_cost = lane_keep_cost + 400*(radius_ego - 101)^2;
    end
