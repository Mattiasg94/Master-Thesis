...
R_t = np.diag([1000000.0, 10000000.0, 100000000.0])
Q_t = np.diag([0.0, 0.0, 0.0])
# -------Init Obstacles
penalty_margin=0.3
...
def animate(i):
    ...
    if mode=='point':
        ...
        # CHOOSE IF U WANT TO PLOT POINTS OR TRAJ.
...
while i<100:
    ...
    ## Choose if we are using traj or point
    ...
    ## DONE chosing if we are using traj or point

    ## Comp time
    total_sec += solution['solve_time_ms']/1000

    ## Initialize variables for MPC loop
    uv = u_star[0:nu*N:2]
    uw = u_star[1:nu*N:2]   
    X = [0.0]*(N+1)
    X[0] = x_init
