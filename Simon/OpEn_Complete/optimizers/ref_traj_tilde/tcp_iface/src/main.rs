///
/// Auto-generated TCP server for optimizer: ref_traj_tilde
/// Generated at: 2020-03-05 16:01:16.699438
///
...
fn execution_handler(
    ...
    match initial_guess {
        ...
        Some(u0) => {
            if u0.len() != REF_TRAJ_TILDE_NUM_DECISION_VARIABLES {
    ...
    if parameter.len() != REF_TRAJ_TILDE_NUM_PARAMETERS {
...
fn run_server() {
    ...
    let mut p = [0.0; REF_TRAJ_TILDE_NUM_PARAMETERS];
    let mut cache = initialize_solver();
    info!("Done");
    let listener = TcpListener::bind(format!("{}:{}", BIND_IP, BIND_PORT)).unwrap();
    let mut u = [0.0; REF_TRAJ_TILDE_NUM_DECISION_VARIABLES];
